...
Critically, the loss is computed \textit{only on I\_target edges}. At inference time the model receives only transistor dimensions and process/PVT parameters.  That is, no other branch currents are available because those could only be obtained through simulation results. Masking the loss to I\_target ensures the gradient signal reflects the \textit{actual} prediction objective and avoids dilution from the five other edge types (I\_vdd, I\_gnd, I\_in, I\_out, and their junction duplicates).
