...
_env_dataset_path = os.environ.get('DATASET_PATH', '').strip()
_env_dataset_name = os.environ.get('DATASET', '').strip()

if _env_dataset_path and os.path.exists(_env_dataset_path):
    DATA_FILE = _env_dataset_path
    print(f"[dataset] using DATASET_PATH: {DATA_FILE}", flush=True)
elif _env_dataset_name:
    _candidate = _env_dataset_name
    if not os.path.isabs(_candidate):
        _candidate = os.path.join(_dataset_dir, _candidate)
    if not _candidate.endswith('.json'):
        _candidate += '.json'
    if os.path.exists(_candidate):
        DATA_FILE = _candidate
        print(f"[dataset] using DATASET: {DATA_FILE}", flush=True)
    else:
        raise FileNotFoundError(f"[dataset] DATASET points to missing file: {_candidate}")
else:
...
print(f"[dataset] {len(np.unique(PROCESS_NAMES))} unique process(es): "
      f"{sorted(set(PROCESS_NAMES.tolist()))[:5]}", flush=True)
...
# =============================================================================
# v2.0 backward-compat: keep the original single global scaler running on
...
print(f"[dataset] PUBLIC_FEATURE_COLS  ({len(PUBLIC_FEATURE_COLS)}): "
      f"{PUBLIC_FEATURE_COLS}", flush=True)
print(f"[dataset] PRIVATE_FEATURE_COLS ({len(PRIVATE_FEATURE_COLS)}): "
      f"{PRIVATE_FEATURE_COLS[:6]}{'...' if len(PRIVATE_FEATURE_COLS) > 6 else ''}",
      flush=True)
