"""
Embedding Inversion Attack Script
Loads embeddings, logits, and attention weights from results/inference_outputs_<process>.npz
and attempts to reconstruct input features from embeddings.
"""
import numpy as np
import argparse
import os
...
def main():
    ...
    # Simple baseline: identity (return embeddings as reconstructed)
    reconstructed = embeddings.copy()
    np.savez(os.path.join(args.privacy_dir, f'embedding_inversion_{args.process}.npz'), reconstructed=reconstructed)
    print("[embedding_inversion] Baseline output saved.")
