...
def main():
    ...
    if args.test:
        current_output = get_squeue_output()
        subject = "SLURM squeue --test output"
    ...
    while True:
        ...
        if args.monitor:
            ...
            subject = "SLURM squeue periodic update"
            body = build_email_body(current_output)
            send_email(subject, body)
        ...
        if current_output != last_output:
            subject = "SLURM squeue change detected"
            body = build_email_body(current_output)
            send_email(subject, body)
            last_output = current_output
            last_job_ids = extract_job_ids(current_output)
