...
#
# Usage:
#   python predict.py --input ../datasets/dataset_22nm_HP.json
#   python predict.py --input ../datasets/my_data.json \
...
def load_checkpoint(checkpoint_path, device):
    ...
    return gan, config, checkpoint  # original: return gan, config, checkpoint["label_log_mean"], checkpoint["label_log_std"], checkpoint["scaler"]
...
def predict_row_monolithic(gan, config, label_log_mean, label_log_std, scaler, feature_columns, raw_row, device):  # original: def predict_row(gan, config, label_log_mean, label_log_std, scaler, feature_columns, raw_row, device):
    ...
    model_row = pd.Series(row)  # original: edges, X = encoder(row, design)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run GAN current prediction inference")
    ...
    fallback_process = os.path.splitext(os.path.basename(args.input))[0]
    ...
    # Name the .npz after the same <process>_<tag> stem as the predictions CSV
    # (predictions file is <process>_<tag>_predictions.csv, so strip that
    # suffix and use <process>_<tag>_inference.npz).
    base = os.path.splitext(os.path.basename(args.output))[0]  # original: process_name = ...replace('predictions_', '')
    if base.endswith('_predictions'):
        base = base[: -len('_predictions')]
    npz_path = os.path.join(os.path.dirname(args.output), f"{base}_inference.npz")  # original: f"inference_outputs_{process_name}.npz"
    np.savez(npz_path,
