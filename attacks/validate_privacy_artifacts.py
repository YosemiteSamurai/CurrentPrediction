...
def resolve_dataset_path(dataset_value, repo_root):
    dataset_dir = repo_root / "datasets"  # original: dataset_dir = repo_root / "dataset"
...
def main():
    ...
    parser.add_argument("--privacy-dir", default=None, help="Directory containing attack artifacts (default: attacks/inputs)")  # original: parser.add_argument("--privacy-dir", default=None, help="Directory containing privacy artifacts")
    parser.add_argument("--results-dir", default=None, help="Directory containing general outputs (default: results/)")
    ...
    privacy_dir = Path(args.privacy_dir) if args.privacy_dir else script_dir / "inputs"  # original: privacy_dir = Path(args.privacy_dir) if args.privacy_dir else script_dir
    ...
    required_files = {
        f"original_embeddings_{process_with_tag}.npz": ("embedding inversion ground truth", privacy_dir),
        f"ground_truth_edges_{process_with_tag}.npz": ("edge reconstruction ground truth", privacy_dir),
        f"membership_labels_{process_with_tag}.npz": ("membership inference labels", privacy_dir),
        f"{process_with_tag}_inference.npz": ("model inference outputs", results_dir),  # original: f"inference_outputs_{process_with_tag}.npz"
    }
    optional_files = {
        f"{process_with_tag}_predictions.csv": ("privacy prediction CSV", results_dir),  # original: f"predictions_{process_with_tag}.csv"
        f"{process_with_tag}_model.pt": ("trained checkpoint", results_dir),  # original: f"model_{process_with_tag}.pt"
    }

    missing_required = []
    for file_name, (description, base_dir) in required_files.items():  # original: for file_name, description in required_files.items():
        file_path = base_dir / file_name  # original: file_path = privacy_dir / file_name
        if file_path.exists():
    ...
    for file_name, (description, base_dir) in optional_files.items():  # original: for file_name, description in optional_files.items():
        file_path = base_dir / file_name  # original: file_path = privacy_dir / file_name
        if file_path.exists():
            print(f"[validate_privacy_artifacts] OK: {file_name} ({description})")
        else:
