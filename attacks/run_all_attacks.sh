...
for RUN_TAG in baseline dp sl both; do
  ...
  "$PYTHON_BIN" privacy_attack.py --process "$PROCESS" --run-tag "$RUN_TAG" --privacy_dir "$INPUTS_DIR" --results_dir "$RESULTS_DIR"
done

echo ""
echo "=== Comparing attacks across baseline/dp/sl/both ==="
"$PYTHON_BIN" compare_attacks.py --process "$PROCESS" --tags baseline dp sl both --privacy_dir "$INPUTS_DIR" > "$INPUTS_DIR/compare_attacks.txt"

echo ""
echo "All attacks completed for ${PROCESS} across baseline/dp/sl/both."
echo "Comparison report written to $INPUTS_DIR/compare_attacks.txt"
